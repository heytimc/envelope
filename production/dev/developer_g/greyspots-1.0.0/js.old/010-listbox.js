...
window.addEventListener('design-register-element', function () {
    registerDesignSnippet('Custom Template Listbox', '<gs-listbox>', 'gs-listbox src="${1:test.tpeople}">\n'+
                                                                     '    <template>\n'+
                                                                     '        <table>\n'+
                                                                     '            <tbody>\n'+
                                                                     '                <tr value="{{! row.id }}">\n'+
                                                                     ...
                                                                     '    </template>\n' +
                                                                     '</gs-listbox>');
    registerDesignSnippet('Dynamic Template Listbox', '<gs-listbox>', 'gs-listbox src="${1:test.tpeople}"></gs-listbox>');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    xtag.register('gs-listbox', {
        lifecycle: {
            ...
            created: function() {
                var element = this, tableTemplateElement, arrElement, recordElement, tableTemplateElementCopy, strQSValue;
                ...
                // handle "qs" attribute
                if (this.hasAttribute('qs') ||
                        this.hasAttribute('refresh-on-querystring-values') ||
                    ...
                    this.popValues = GS.qryToJSON(GS.getQueryString());
                }
                ...
                // select for template
                tableTemplateElement = xtag.queryChildren(this, 'template')[0];
                ...
                if (tableTemplateElement) {
                    ...
                    this.tableTemplate = tableTemplateElementCopy.innerHTML;
                }
                
                if (this.getAttribute('src') || this.getAttribute('source')) {
                    this.getData(true);
                } else {
                    if (tableTemplateElement) {
                        this.tableElement = xtag.query(tableTemplateElementCopy.content, 'table')[0];
                    } else if (xtag.queryChildren(this, 'table')[0]) {
                        this.tableElement = xtag.queryChildren(this, 'table')[0];
                    } else {
            ...
            attributeChanged: function(strAttrName, oldVal, newVal) {
                if (strAttrName === 'value' && newVal !== null && this.tableElement) {
                    this.selectRecord(newVal);
                    if (!this.hackToPreventScroll) {
        ...
        methods: {
            ...
            highlightRecord: function (record) {
                ...
                arrSelectedTrs = xtag.queryChildren(xtag.queryChildren(this.tableElement, 'tbody')[0], 'tr[selected]');
            ...
            findRecordFromValue: function (searchValue) {
                var i, len, matchedRecord, arrTrs = xtag.queryChildren(xtag.queryChildren(this.tableElement, 'tbody')[0], 'tr'),
            ...
            selectRecord: function (handle, bolChange) {
                var record, strRecordValue, strFirstTdText;
                
                if (!this.hasAttribute('no-select')) {
                    if (typeof handle === 'string' || typeof handle === 'number') {
                        record = this.findRecordFromValue(handle);
                    } else {
                        record = handle;
                    }
                    ...
                    if (!record && handle !== '') {
                        ...
                        if (this.value !== (strRecordValue || strFirstTdText)) {
                            this.innerValue = strRecordValue || strFirstTdText;
                            this.innerSelectedRecord = record;
                            if (bolChange) {
                                this.hackToPreventScroll = true;
                                this.setAttribute('value', this.innerValue);
                                this.hackToPreventScroll = false;
                                this.triggerChange();
                    ...
                    //console.log('3***', this.selectedRecord, this.value);
                }
            },
                    ...
                    if (!this.hasAttribute('no-select')) {
                        if ((intKeyCode === 40 || intKeyCode === 38) && !event.shiftKey && !event.metaKey && !event.ctrlKey && !element.error) {
                            ...
                            if (element.tableElement && selectedTr) {
                                element.selectRecord(selectedTr, true);
                            }
                        }
                    }
                } else {
            ...
            handleFocusout: function () {
                var selectedTr = xtag.queryChildren(xtag.queryChildren(this.tableElement, 'tbody')[0], 'tr[selected]')[0];
                
                if (selectedTr) {
                    this.selectRecord(selectedTr, true);
                }
            },
            ...
            //      else
            //          use: source query
            getData: function (bolInitalLoad, bolClearPrevious) {
                var element = this, strLink,
                ...
                strLink = '/v1/' + (element.getAttribute('action-select') || 'env/action_select') +
                            ...
                            '&where='    + encodeURIComponent(queryStringTemplate(decodeURIComponent(this.getAttribute('where') || ''))) +
                            '&limit='    + encodeURIComponent(queryStringTemplate(decodeURIComponent(this.getAttribute('limit') || ''))) +
                            '&offset='   + encodeURIComponent(queryStringTemplate(decodeURIComponent(this.getAttribute('offset') || ''))) +
                            '&order_by=' + encodeURIComponent(queryStringTemplate(decodeURIComponent(this.getAttribute('ord') || ''))) +
                            '&cols='     + encodeURIComponent(this.getAttribute('cols') || '');
            ...
            handleData: function (bolInitalLoad, data, error) {
                var element = this, strTemplate, divElement, tableElement, theadElement, theadCellElements, tbodyElement, tbodyCellElements, lastRecordElement,
                    recordElements, currentCellLabelElement, template, i, len, arrHeaders = [], arrHide, intVisibleColumns, strHeaderCells, strRecordCells;
                ...
                if (!error) {
                    ...
                    //    element.selectRecord(element.getAttribute('value'), false);
                    //    element.scrollToSelectedRecord();
                    //    
                    //// select first record
                    //} else
                    ...
                    if (bolInitalLoad && !element.getAttribute('value') && element.hasAttribute('select-first')) {
                        element.selectRecord(xtag.query(element, 'tbody tr')[0].getAttribute('value'), false);
                        element.scrollToSelectedRecord();
                    }
                ...
                if (element.clientHeight < element.scrollContainer.scrollHeight) {
                    ...
                    element.mousemoveHandler = function (event) {
                        ...
                        if (event.which !== 0 || evt.touchDevice) {
                            ...
                            window.removeEventListener(evt.mousemove, arguments.callee);
                        }
                    };
            ...
            syncView: function () {
                ...
                if (this.getAttribute('value')) {
                    this.selectRecord(this.getAttribute('value'));
                    this.scrollToSelectedRecord();
                }
                ...
                clickHandler = function (event) {
                    ...
                    element.selectRecord(this, true);
                };
                ...
                //        element.selectRecord(parentRecord, true);
                //    }
                //});
            },
