...
window.addEventListener('load', function () {
    'use strict';
    var bolOpen, intMaxHeight, curlElement, menuElement, strPostageUName, strHTML;
    ...
    if (window.bolCurl !== false) {
        ...
        if (evt.deviceType === 'phone') {
            curlElement.setAttribute('style', 'font-size: 1.8em;');
        } else {
        ...
        curlElement.addEventListener('click', function () {
            var intFontSize = GS.pxToEm(document.body, window.innerWidth) / 4,
                ...
                closedSize = (evt.deviceType === 'phone' ? '1.8em' : '0.7em'); // replace evt.touchDevice with true to test on a desktop
...
window.addEventListener('load', function () {
    ...
    jsnShimmed.matchesSelector  = !nativeTest(Element.prototype.matchesSelector) &&
    ...
    jsnShimmed.MutationObserver = !nativeTest(window.MutationObserver);
    jsnShimmed.WeakMap          = !nativeTest(window.WeakMap);
    jsnShimmed.registerElement  = !nativeTest(document.registerElement);
    jsnShimmed.DOMTokenList     = !nativeTest(window.DOMTokenList);
