window.addEventListener('design-register-element', function (event) {
    ...
    window.designElementProperty_GSENVELOPE = function (selectedElement) {
        ...
        addProp('Scroll To Bottom', true, '<gs-text class="target" value="' + (selectedElement.getAttribute('scroll-to-bottom') || '') + '" mini></gs-text>', function () {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function getData(element) {
        var data, strLink, strOrderBy = '', strWhere, strWhereColumn, strSelectAction,
            strWhereAttribute = queryStringTemplate((element.getAttribute('where') || '')),
            strSource = queryStringTemplate(decodeURIComponent(element.getAttribute('src') ||
                                                                    element.getAttribute('source') || '')),
            strCols = queryStringTemplate(element.getAttribute('cols') || ''), callbackFunction, i, len;
        ...
        // else: use the order by attribute
            ...
            strOrderBy = queryStringTemplate(decodeURIComponent(element.getAttribute('ord') || ''));
        }
        ...
        if (!error) {
            ...
            if (!theadElement) {
                ...
                for (i = 0, len = arrCells.length, strHeaderHTML = '', strFixedHeaderHTML = ''; i < len; i += 1) {
                    strCurrentHeadingText = encodeHTML(queryStringTemplate(arrCells[i].getAttribute('heading') || ''));
            ...
            // refresh height, fixed header widths and reflow status
    ...
    function updateRecord(element, record, strColumn, newValue) {
        ...
        callbackFunction = function (event) {
            var jsnData, i, len, idIndex, tbodyElement, recordIndex, focusElement = document.activeElement,
                ...
                focusElementIndex, focusElementCell, elementWalkResult;
    ...
    function insertRecord(element, strInsertString) {
                ...
                GS.ajaxErrorDialog(response, function () {
                    ...
                    errorCallbackFunction = function () {
                        ...
                        for (var i = 0, len = arrKey.length; i < len; i += 1) {
                            ...
                            } else */if (elem.refresh) {
                                // Callback sets all the values for the inputs
                                elem.refresh(true, true, setValues);
                            }
                        }
        ...
        methods: {
            ...
            refreshHeight: function () {
                ...
                if (this.hasAttribute('expand-to-content')) {
                    ...
                    this.style.height = intHeight + 'px';
                }
            }
        }
    });
