...
//              )
//      )
GS.qryToWhere = function (strQueryString, strColumnNameInQueryString, strColumnNameInTarget) {
    'use strict';
    var strWhere = '', key, jsnArgs;
    
    if (strColumnNameInQueryString) {
        strColumnNameInTarget = (strColumnNameInTarget || strColumnNameInQueryString);
        
        if (!isNaN(GS.qryGetVal(strQueryString, strColumnNameInQueryString))) {
            strWhere = strColumnNameInTarget + '=' + GS.qryGetVal(strQueryString, strColumnNameInQueryString);
        } else {
            strWhere = strColumnNameInTarget + '::text = \'' + encodeURIComponent(GS.qryGetVal(strQueryString, strColumnNameInQueryString)) + '\'::text';
        }
        
    } else {
        jsnArgs = GS.qryToJSON(strQueryString);
        ...
        for (key in jsnArgs) {
            if (jsnArgs.hasOwnProperty(key)) {
                ...
                if (!isNaN(jsnArgs[key])) {
                    ...
                    strWhere += (strWhere === '' ? '': ' AND ') + key + '::text = \'' + encodeURIComponent(jsnArgs[key]) + '\'::text';
                }
            }
        }
    }
